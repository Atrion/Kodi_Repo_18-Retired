...
def get_tvshow_window(window_type):
	...
	class DialogTVShowInfo(DialogBaseInfo, window_type):
		...
		def browse_tvshow(self):
			url = 'plugin://plugin.video.x-wing/tv/tvdb/%s/' % self.info['tvdb_id']
			self.close()
		...
		def play_tvshow(self):
			url = 'plugin://plugin.video.x-wing/tv/play/%s/1/1' % self.info['tvdb_id']
			PLAYER.play_from_button(url, listitem=None, window=self, dbid=0)

		@ch.click(20)
		def add_tvshow_to_library(self):
			if xbmcgui.Dialog().yesno('[B][COLOR lightseagreen]D[/COLOR]eath[COLOR  lightseagreen]S[/COLOR]tar EIM[/B]', 'Add [B]%s[/B] to library?' % self.info['TVShowTitle']):
				xbmc.executebuiltin('RunPlugin(plugin://plugin.video.x-wing/tv/add_to_library/%s)' % self.info['tvdb_id'])
		...
		def remove_tvshow_from_library(self):
			if xbmcgui.Dialog().yesno('[B][COLOR lightseagreen]D[/COLOR]eath[COLOR  lightseagreen]S[/COLOR]tar EIM[/B]', 'Remove [B]%s[/B] from library?' % self.info['TVShowTitle']):
