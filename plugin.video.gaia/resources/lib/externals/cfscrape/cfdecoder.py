...
class Cloudflare:
    ...
    def get_url(self):
        ...
        if self.js_data.get("wait", 0):
            ...
            response = "%s://%s%s?%s" % (
                self.protocol, self.domain, self.js_data["auth_url"], urlencode(self.js_data["params"]))
        ...
        if self.header_data.get("wait", 0):
            response = "%s://%s%s?%s" % (
                self.protocol, self.domain, self.header_data["auth_url"], urlencode(self.header_data["params"]))
