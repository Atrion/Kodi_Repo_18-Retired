...
PlaScrapersSettings = None
PlaScrapersProviders = None

class source:

	SettingLabel = 'providers.external.universal.open.plascrapersx.providers.label'
	SettingValue = 'providers.external.universal.open.plascrapersx.providers'
	...
	def _instancesPath(self):
		return tools.System.pathProviders('PlacentaScrapers')
	...
	@classmethod
	def instances(self):
		global PlaScrapersSettings
		if PlaScrapersSettings == None:
			tools.System.addon(tools.Extensions.IdPlacenta).setSetting('_%s_' % tools.System.name().lower(), '') # Forces Kodi to generate the settings profile file if it does not already exist.
			PlaScrapersSettings = tools.File.readNow(tools.File.joinPath(tools.System.profile(tools.Extensions.IdPlacenta), 'settings.xml'))
	...
	def instancesProviders(self):
		global PlaScrapersProviders
		if PlaScrapersProviders == None:
			PlaScrapersProviders = tools.Settings.getObject(self.SettingValue)
			if not PlaScrapersProviders: PlaScrapersProviders = {}
		return PlaScrapersProviders
	...
	def instancesSettings(self):
		...
		addon = copy.deepcopy(PlaScrapersProviders)
		...
		for i in range(len(instances)):
			...
			if not instance.id in PlaScrapersProviders: PlaScrapersProviders[instance.id] = instance.enabled
		...
		while True:
			...
			for i in range(len(labels)):
				items.append(labels[i] + (enabled if PlaScrapersProviders[ids[i]] else disabled))
			choice = interface.Dialog.select(title = 32345, items = items)

			if choice <= 0: break
			elif choice == 1: PlaScrapersProviders = {i : False for i in PlaScrapersProviders}
			elif choice == 2: PlaScrapersProviders = {i : True for i in PlaScrapersProviders}
			elif choice == 3: PlaScrapersProviders = copy.deepcopy(addon)
			else:
				index = choice - 4 - len(languages)
				if index >= 0:
					id = ids[index]
					PlaScrapersProviders[id] = not PlaScrapersProviders[id]
				else:
					language = languages[choice - 4]
					for i in instances:
						if i.language[0] == language:
							PlaScrapersProviders[i.id] = True

			tools.Settings.set(self.SettingLabel, str(sum(PlaScrapersProviders.values())) + ' ' + interface.Translation.string(32301))
			tools.Settings.set(self.SettingValue, PlaScrapersProviders)
	...
	def instancesEnable(self, providers, enable = True):
		...
		for i in range(len(PlaScrapersProviders)):
			for j in providers:
				if j in PlaScrapersProviders:
					PlaScrapersProviders[j] = enable
					if single:
						found = True
						break
