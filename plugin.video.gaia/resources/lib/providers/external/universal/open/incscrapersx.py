...
IncScrapersSettings = None
IncScrapersProviders = None

class source:

	SettingLabel = 'providers.external.universal.open.incscrapersx.providers.label'
	SettingValue = 'providers.external.universal.open.incscrapersx.providers'

	def __init__(self):
		self.addon = 'Incursion'
		self.priority = 1
		self.language = ['un']
		self.domains = []
		self.base_link = ''
	...
	def _instancesPath(self):
		return tools.System.pathProviders('IncursionScrapers')
	...
	def _instancesRename(self, path):
		replacements = [
			['from resources.', 'from incursion.'],
			['xbmcaddon.Addon()', 'xbmcaddon.Addon("' + tools.Extensions.IdIncursion + '")'],
	...
	@classmethod
	def instances(self):
		global IncScrapersSettings
		if IncScrapersSettings == None:
			tools.System.addon(tools.Extensions.IdIncursion).setSetting('_%s_' % tools.System.name().lower(), '') # Forces Kodi to generate the settings profile file if it does not already exist.
			IncScrapersSettings = tools.File.readNow(tools.File.joinPath(tools.System.profile(tools.Extensions.IdIncursion), 'settings.xml'))
					...
					for package2, name2, pkg2 in pkgutil.walk_packages(path2):
						if not pkg2:
							try:
								...
								scraperNew.enabled = tools.Settings.raw('provider.' + id, parameter = tools.Settings.ParameterValue, data = IncScrapersSettings)
	...
	def instancesProviders(self):
		global IncScrapersProviders
		if IncScrapersProviders == None:
			IncScrapersProviders = tools.Settings.getObject(self.SettingValue)
			if not IncScrapersProviders: IncScrapersProviders = {}
		return IncScrapersProviders
	...
	def instancesSettings(self):
		...
		addon = copy.deepcopy(IncScrapersProviders)
		...
		for i in range(len(instances)):
			...
			if not instance.id in IncScrapersProviders: IncScrapersProviders[instance.id] = instance.enabled
		...
		while True:
			...
			for i in range(len(labels)):
				items.append(labels[i] + (enabled if IncScrapersProviders[ids[i]] else disabled))
			choice = interface.Dialog.select(title = 32345, items = items)

			if choice <= 0: break
			elif choice == 1: IncScrapersProviders = {i : False for i in IncScrapersProviders}
			elif choice == 2: IncScrapersProviders = {i : True for i in IncScrapersProviders}
			elif choice == 3: IncScrapersProviders = copy.deepcopy(addon)
			else:
				index = choice - 4 - len(languages)
				if index >= 0:
					id = ids[index]
					IncScrapersProviders[id] = not IncScrapersProviders[id]
				else:
					language = languages[choice - 4]
					for i in instances:
						if i.language[0] == language:
							IncScrapersProviders[i.id] = True

			tools.Settings.set(self.SettingLabel, str(sum(IncScrapersProviders.values())) + ' ' + interface.Translation.string(32301))
			tools.Settings.set(self.SettingValue, IncScrapersProviders)
	...
	def instancesEnable(self, providers, enable = True):
		...
		for i in range(len(IncScrapersProviders)):
			for j in providers:
				if j in IncScrapersProviders:
					IncScrapersProviders[j] = enable
					if single:
						found = True
						break
