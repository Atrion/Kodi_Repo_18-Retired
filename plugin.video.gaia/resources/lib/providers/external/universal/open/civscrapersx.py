...
CivScrapersSettings = None
CivScrapersProviders = None

class source:

	SettingLabel = 'providers.external.universal.open.civscrapersx.providers.label'
	SettingValue = 'providers.external.universal.open.civscrapersx.providers'

	def __init__(self):
		self.addon = 'CivitasScrapers'
		self.priority = 1
		self.language = ['un']
		self.domains = []
		self.base_link = ''
	...
	def _instancesPath(self):
		return tools.System.pathProviders('CivitasScrapers')
	...
	def _instancesPrepare(self):
		pathSource = tools.System.path(tools.Extensions.IdCivScrapers)
		...
		self._instancesInclude()
		return tools.File.joinPath(pathDestination, 'lib', 'civitasscrapers', 'sources_civitasscrapers')

	@classmethod
	def instances(self):
		global CivScrapersSettings
		if CivScrapersSettings == None:
			tools.System.addon(tools.Extensions.IdCivScrapers).setSetting('_%s_' % tools.System.name().lower(), '') # Forces Kodi to generate the settings profile file if it does not already exist.
			CivScrapersSettings = tools.File.readNow(tools.File.joinPath(tools.System.profile(tools.Extensions.IdCivScrapers), 'settings.xml'))
					...
					for package2, name2, pkg2 in pkgutil.walk_packages(path2):
						if not pkg2:
							try:
								...
								scraperNew.enabled = tools.Settings.raw('provider.' + id, parameter = tools.Settings.ParameterValue, data = CivScrapersSettings)
	...
	def instancesProviders(self):
		global CivScrapersProviders
		if CivScrapersProviders == None:
			CivScrapersProviders = tools.Settings.getObject(self.SettingValue)
			if not CivScrapersProviders: CivScrapersProviders = {}
		return CivScrapersProviders
	...
	def instancesSettings(self):
		...
		addon = copy.deepcopy(CivScrapersProviders)
		...
		for i in range(len(instances)):
			...
			if not instance.id in CivScrapersProviders: CivScrapersProviders[instance.id] = instance.enabled
		...
		while True:
			...
			for i in range(len(labels)):
				items.append(labels[i] + (enabled if CivScrapersProviders[ids[i]] else disabled))
			choice = interface.Dialog.select(title = 32345, items = items)

			if choice <= 0: break
			elif choice == 1: CivScrapersProviders = {i : False for i in CivScrapersProviders}
			elif choice == 2: CivScrapersProviders = {i : True for i in CivScrapersProviders}
			elif choice == 3: CivScrapersProviders = copy.deepcopy(addon)
			else:
				index = choice - 4 - len(languages)
				if index >= 0:
					id = ids[index]
					CivScrapersProviders[id] = not CivScrapersProviders[id]
				else:
					language = languages[choice - 4]
					for i in instances:
						if i.language[0] == language:
							CivScrapersProviders[i.id] = True

			tools.Settings.set(self.SettingLabel, str(sum(CivScrapersProviders.values())) + ' ' + interface.Translation.string(32301))
			tools.Settings.set(self.SettingValue, CivScrapersProviders)
	...
	def instancesEnable(self, providers, enable = True):
		...
		for i in range(len(CivScrapersProviders)):
			for j in providers:
				if j in CivScrapersProviders:
					CivScrapersProviders[j] = enable
					if single:
						found = True
						break
