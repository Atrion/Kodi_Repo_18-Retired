...
OpeScrapersSettings = None
OpeScrapersProviders = None

class source:

	SettingLabel = 'providers.external.universal.open.opescrapersx.providers.label'
	SettingValue = 'providers.external.universal.open.opescrapersx.providers'

	def __init__(self):
		self.addon = 'OpenScrapers'
		self.priority = 1
		self.language = ['un']
		self.domains = []
		self.base_link = ''
	...
	def _instancesPath(self):
		return tools.System.pathProviders('OpenScrapers')
	...
	def _instancesRename(self, path):
		...
		replacements = [['from resources.lib.', 'from openscrapers.'], ['from openscrapers.modules import cfscrape', 'try: from openscrapers.modules import cfscrape\nexcept: pass'], ['xbmcaddon.Addon()', 'xbmcaddon.Addon("' + tools.Extensions.IdOpeScrapers + '")']]
	...
	def _instancesPrepare(self):
		pathSource = tools.System.path(tools.Extensions.IdOpeScrapers)
		...
		self._instancesInclude()
		return tools.File.joinPath(pathDestination, 'lib', 'openscrapers', 'sources_openscrapers')

	@classmethod
	def instances(self):
		global OpeScrapersSettings
		if OpeScrapersSettings == None:
			tools.System.addon(tools.Extensions.IdOpeScrapers).setSetting('_%s_' % tools.System.name().lower(), '') # Forces Kodi to generate the settings profile file if it does not already exist.
			OpeScrapersSettings = tools.File.readNow(tools.File.joinPath(tools.System.profile(tools.Extensions.IdOpeScrapers), 'settings.xml'))
					...
					for package2, name2, pkg2 in pkgutil.walk_packages(path2):
						if not pkg2:
							try:
								...
								scraperNew.enabled = tools.Settings.raw('provider.' + id, parameter = tools.Settings.ParameterValue, data = OpeScrapersSettings)
	...
	def instancesProviders(self):
		global OpeScrapersProviders
		if OpeScrapersProviders == None:
			OpeScrapersProviders = tools.Settings.getObject(self.SettingValue)
			if not OpeScrapersProviders: OpeScrapersProviders = {}
		return OpeScrapersProviders
	...
	def instancesSettings(self):
		...
		addon = copy.deepcopy(OpeScrapersProviders)
		...
		for i in range(len(instances)):
			...
			if not instance.id in OpeScrapersProviders: OpeScrapersProviders[instance.id] = instance.enabled
		...
		while True:
			...
			for i in range(len(labels)):
				items.append(labels[i] + (enabled if OpeScrapersProviders[ids[i]] else disabled))
			choice = interface.Dialog.select(title = 32345, items = items)

			if choice <= 0: break
			elif choice == 1: OpeScrapersProviders = {i : False for i in OpeScrapersProviders}
			elif choice == 2: OpeScrapersProviders = {i : True for i in OpeScrapersProviders}
			elif choice == 3: OpeScrapersProviders = copy.deepcopy(addon)
			else:
				index = choice - 4 - len(languages)
				if index >= 0:
					id = ids[index]
					OpeScrapersProviders[id] = not OpeScrapersProviders[id]
				else:
					language = languages[choice - 4]
					for i in instances:
						if i.language[0] == language:
							OpeScrapersProviders[i.id] = True

			tools.Settings.set(self.SettingLabel, str(sum(OpeScrapersProviders.values())) + ' ' + interface.Translation.string(32301))
			tools.Settings.set(self.SettingValue, OpeScrapersProviders)
	...
	def instancesEnable(self, providers, enable = True):
		...
		for i in range(len(OpeScrapersProviders)):
			for j in providers:
				if j in OpeScrapersProviders:
					OpeScrapersProviders[j] = enable
					if single:
						found = True
						break
