...
UniScrapersProviders = None

class source:

	SettingLabel = 'providers.external.universal.open.uniscrapersx.providers.label'
	SettingValue = 'providers.external.universal.open.uniscrapersx.providers'

	def __init__(self):
		self.addon = 'UniversalScrapers'
		self.priority = 1
		self.language = ['un']
		self.domains = []
		self.base_link = ''
	...
	def instancesProviders(self):
		global UniScrapersProviders
		if UniScrapersProviders == None:
			UniScrapersProviders = tools.Settings.getObject(self.SettingValue)
			if not UniScrapersProviders: UniScrapersProviders = {}
		return UniScrapersProviders
	...
	def instancesSettings(self):
		...
		addon = copy.deepcopy(UniScrapersProviders)
		...
		for i in range(len(instances)):
			...
			if not instance.id in UniScrapersProviders: UniScrapersProviders[instance.id] = instance.enabled
		...
		while True:
			...
			for i in range(len(labels)):
				items.append(labels[i] + (enabled if UniScrapersProviders[ids[i]] else disabled))
			choice = interface.Dialog.select(title = 32345, items = items)

			if choice <= 0: break
			elif choice == 1: UniScrapersProviders = {i : False for i in UniScrapersProviders}
			elif choice == 2: UniScrapersProviders = {i : True for i in UniScrapersProviders}
			elif choice == 3: UniScrapersProviders = copy.deepcopy(addon)
			else:
				index = choice - 4 - len(languages)
				if index >= 0:
					id = ids[index]
					UniScrapersProviders[id] = not UniScrapersProviders[id]
				else:
					language = languages[choice - 4]
					for i in instances:
						if i.language[0] == language:
							UniScrapersProviders[i.id] = True

			tools.Settings.set(self.SettingLabel, str(sum(UniScrapersProviders.values())) + ' ' + interface.Translation.string(32301))
			tools.Settings.set(self.SettingValue, UniScrapersProviders)
	...
	def instancesEnable(self, providers, enable = True):
		...
		for i in range(len(UniScrapersProviders)):
			for j in providers:
				if j in UniScrapersProviders:
					UniScrapersProviders[j] = enable
					if single:
						found = True
						break
