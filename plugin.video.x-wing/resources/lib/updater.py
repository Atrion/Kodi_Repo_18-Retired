...
def update_players_local(zip_path):
	extract_to = xbmc.translatePath('special://profile/addon_data/plugin.video.x-wing/Players')  
...
def flat_extract(z, extract_to, members=None):
	...
	if not os.path.exists(extract_to):
		...
		if xbmc.getInfoLabel('Window(home).Property(running)') == 'totalx-wing':
			...
			if plugin.yesno('[B][COLOR lightseagreen]M[/COLOR]x-win[COLOR lightseagreen]G[/COLOR] D[COLOR lightseagreen]S[/COLOR][/B]: Update players', 'Do you want to remove existing players first?'):
				empty_folder(extract_to)
