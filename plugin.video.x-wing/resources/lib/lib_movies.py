...
def setup_library(library_folder):
	...
	if not xbmcvfs.exists(library_folder):
		...
		msg = 'Would you like to automatically set [B][COLOR lightseagreen]M[/COLOR]x-win[COLOR lightseagreen]G[/COLOR] D[COLOR lightseagreen]S[/COLOR][/B] as a movies video source?'
		if plugin.yesno('Library setup', msg):
			source_thumbnail = plugin.get_media_icon('movies')
			source_name = '[B][COLOR lightseagreen]M[/COLOR]x-win[COLOR lightseagreen]G[/COLOR] D[COLOR lightseagreen]S[/COLOR][/B] Movies'
